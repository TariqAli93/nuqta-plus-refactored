...
REM    6. Frontend desktop app is launched (if present)
REM
...
"%PG_BIN%\psql.exe" -h "%PG_HOST%" -p "%PG_PORT%" -U "%PG_USER%" -d postgres -tAc "SELECT 1" >nul 2>&1
if errorlevel 1 (
...
if "%DB_EXISTS%"=="1" (
  ...
  "%PG_BIN%\createdb.exe" -h "%PG_HOST%" -p "%PG_PORT%" -U "%PG_USER%" "%PG_DATABASE%"
  if errorlevel 1 (
