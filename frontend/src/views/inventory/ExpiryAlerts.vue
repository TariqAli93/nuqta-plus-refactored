<template>
  <div class="page-shell">
    ...
    <v-card class="page-section pa-3">
      ...
      <div class="d-flex justify-end"><v-btn color="primary" @click="load">تحديث</v-btn></div>
    </v-card>
...
const load = async () => {
  loading.value = true;
  try {
    rows.value = await inventoryStore.fetchExpiryAlerts({ warehouseId: filters.value.warehouseId || undefined, status: filters.value.status || undefined });
